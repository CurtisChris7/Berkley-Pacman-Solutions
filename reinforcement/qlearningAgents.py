...
class QLearningAgent(ReinforcementAgent):
    ...
    def getAction(self, state):
        ...
        action = None
        "*** YOUR CODE HERE ***"
        if len(legalActions) == 0:  # Early return
          return action
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        features = featureVector.keys()

        total = 0
        for feature in features:
          weight = self.getWeights().get(feature) 
          if weight is None:
            total += 0
          else:
            total += self.getWeights().get(feature) * featureVector[feature]
        return total
    ...
    def update(self, state, action, nextState, reward):
        ...
        for feature in features:
          weights[feature] = self.getWeights()[feature] + (self.alpha * difference * featureVector[feature])
